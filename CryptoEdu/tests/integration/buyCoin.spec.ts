...
test('selects Bitcoin and places a buy bid of 0.01 BTC', async ({ page }) => {
  ...
  const buyTab = page.getByRole('button', { name: 'Buy', exact: true });
  await buyTab.click();
  ...
  // Submit buy
  const buySubmitButton = page.locator('button[type="submit"]');
  await buySubmitButton.click();
  ...
  await expect(page.locator('text=Successfully bought 0.01 bitcoin')).toBeVisible({ timeout: 10000 });
});
